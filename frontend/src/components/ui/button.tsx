...
/**
 * Glass-refraction button. Uses an SVG `feDisplacementMap` filter to
 * distort the pixels underneath it (Chromium-only — Safari/Firefox
 * fall back gracefully to just the glossy bevel).
 *
 * IMPORTANT: the SVG filter `<GlassFilter />` only needs to exist
 * ONCE on the page. If you render multiple LiquidButtons on the same
 * screen, mount `<GlassFilter />` manually at the top of the layout
 * and pass `withFilter={false}` here to avoid duplicate element IDs.
 */
function LiquidButton({
  ...
  VariantProps<typeof liquidbuttonVariants> & {
    asChild?: boolean;
    /** Render the shared `<GlassFilter />` alongside the button. Disable
     *  when you've already mounted the filter elsewhere. */
    withFilter?: boolean;
  }) {
  const Comp = asChild ? Slot : "button";
  ...
  return (
    <>
      ...
      >
        ...
        />
        <div className="pointer-events-none relative z-10 inline-flex items-center gap-[inherit] whitespace-nowrap">
          {children}
        </div>
      </Comp>
      {withFilter && <GlassFilter />}
    </>
  );
}

/**
 * SVG filter definition used by `LiquidButton`. Render exactly once
 * per page (either automatically via `LiquidButton` with `withFilter`,
 * or manually at a high-level layout).
 */
